...
mulesoft_sonnar_rules = MuleSoftSonarRules()
...
class MulesoftCodeReviewAgent():
    ...
    def AgentConfig(self):
        print("Configuring Mulesoft Code Review Agent...")
        open_router_model_client = self._ModelConfig()

        assistant_agent = AssistantAgent(
            name = 'mulesoft_code_review_agent',
            model_client = open_router_model_client,
            tools=[mulesoft_sonnar_rules.analyze_code()],
            system_message="""
              You are an intelligent code review assistant agent, dedicated to reviewing MuleSoft code files, including .xml, .dwl, and .json formats.
            """
